...
import { getComparativeValuationBySymbol } from '../../../../api/Symbol'
import abbreviateNumber from '../../../../Common/NumberFormat'

const ComparativeValuationGenerator = ({
  data,
  columnList,
  loading,
  industry,
  ...
  useEffect(() => {
    if (data && columnList && industry && sectorData) {
      ...
      const industryMedian = data?.in_05
      const tempData = columnList.map((column, i) => {
        ...
        newObj.industryMedian =
          sectorData &&
          sectorData['industry'] &&
          sectorData['industry']['0.5'] &&
          sectorData['industry']['0.5'][column.key] &&
          sectorData['industry']['0.5'][column.key][industry] &&
          parseFloat(
            sectorData['industry']['0.5'][column.key][industry].toFixed(2)
          )
        let key = ''
  ...
  const changeColumn = async (ticker) => {
    if (ticker && currentClickedColumn) {
      try {
        const resp = await getComparativeValuationBySymbol({ symbol: ticker })
        if (resp) {
