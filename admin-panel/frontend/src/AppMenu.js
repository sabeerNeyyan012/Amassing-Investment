...
const AppSubmenu = (props) => {
    ...
    const renderLink = (item, i) => {
        ...
        if (item.to) {
            return (
                <NavLink aria-label={item.label} onKeyDown={onKeyDown} role="menuitem" className="p-ripple" activeclassname="router-link-active router-link-exact-active" to={item.to} onClick={(e) => onMenuItemClick(e, item, i)} exact={"true"} target={item.target}>
                    {content}
                </NavLink>
            )
        }
